...
import {
  TextInput, Text, StyleSheet, Pressable, ScrollView,
...
const ROLES = ['Officer', 'Technician', 'HeadOfficer'] as const;

const RegisterOfficer = ({ navigation }: any) => {
  const [formData, setFormData] = useState({
    numberId: '', firstName: '', lastName: '', phoneNumber: '',
    email: '', password: '', confirmPassword: '', role: 'Officer' as typeof ROLES[number],
  });
  ...
  const validate = () => {
    const { numberId, firstName, lastName, phoneNumber, email, password, confirmPassword, role } = formData;
    ...
    if (!ROLES.includes(role as any)) return 'กรุณาเลือกบทบาทให้ถูกต้อง';
    return '';
  };
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.titleWrap}>
          <Text style={styles.title}>Create new Officer Account</Text>
          <Text style={styles.subtitle}>สร้างบัญชีเจ้าหน้าที่ • ระบุบทบาทและข้อมูลติดต่อ</Text>
        </View>
